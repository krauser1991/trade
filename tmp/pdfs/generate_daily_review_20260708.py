...
MD_PATH = ROOT / "daily-reviews/2026-07-08-盘后复盘与2026-07-09操作指引.md"
PDF_PATH = ROOT / "output/pdf/2026-07-08-盘后复盘与2026-07-09操作指引.pdf"
...
def build_styles():
    ...
    styles.add(ParagraphStyle(name="TitleCN", fontName="CN", fontSize=18.5, leading=25, alignment=TA_CENTER, textColor=colors.HexColor("#172033"), spaceAfter=12))
    styles.add(ParagraphStyle(name="H2CN", fontName="CN", fontSize=13.6, leading=18.5, textColor=colors.HexColor("#0f3f6f"), spaceBefore=8, spaceAfter=5))
    styles.add(ParagraphStyle(name="H3CN", fontName="CN", fontSize=11.3, leading=15.7, textColor=colors.HexColor("#17324d"), spaceBefore=6, spaceAfter=4))
    styles.add(ParagraphStyle(name="BodyCN", fontName="CN", fontSize=8.7, leading=13.1, alignment=TA_LEFT, textColor=colors.HexColor("#222222"), spaceAfter=3.8))
    styles.add(ParagraphStyle(name="BulletCN", fontName="CN", fontSize=8.7, leading=13.1, leftIndent=12, firstLineIndent=-8, spaceAfter=3.2))
    styles.add(ParagraphStyle(name="CodeCN", fontName="CodeFont", fontSize=8.1, leading=11.4, leftIndent=8, rightIndent=8, textColor=colors.HexColor("#263238"), backColor=colors.HexColor("#f4f6f8"), borderPadding=6, spaceAfter=6))
    styles.add(ParagraphStyle(name="SmallCN", fontName="CN", fontSize=6.55, leading=8.8, textColor=colors.HexColor("#333333")))
    return styles


def table_widths(col_count: int, width: float):
    if col_count == 10:
        return [width * x for x in [0.08, 0.07, 0.11, 0.08, 0.08, 0.12, 0.10, 0.08, 0.08, 0.20]]
    if col_count == 9:
        return [width * x for x in [0.10, 0.09, 0.12, 0.09, 0.09, 0.13, 0.11, 0.10, 0.17]]
    if col_count == 8:
        return [width * x for x in [0.13, 0.13, 0.11, 0.11, 0.14, 0.13, 0.10, 0.15]]
    if col_count == 7:
...
def build_pdf() -> None:
    ...
    doc = SimpleDocTemplate(str(PDF_PATH), pagesize=A4, rightMargin=11 * mm, leftMargin=11 * mm, topMargin=14 * mm, bottomMargin=14 * mm)
    styles = build_styles()
    story = []
