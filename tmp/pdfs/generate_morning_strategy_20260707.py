...
MD_PATH = ROOT / "strategies/2026-07-07-隔夜消息面与早盘交易指引.md"
PDF_PATH = ROOT / "output/pdf/2026-07-07-隔夜消息面与早盘交易指引.pdf"
...
def build_styles():
    ...
    styles.add(ParagraphStyle(name="SmallCN", fontName="CN", fontSize=6.8, leading=9.3, textColor=colors.HexColor("#333333")))
    return styles


def table_widths(col_count: int, width: float):
    if col_count == 5:
        return [width * x for x in [0.12, 0.20, 0.24, 0.22, 0.22]]
    if col_count == 4:
        return [width * x for x in [0.18, 0.24, 0.24, 0.34]]
    if col_count == 3:
        return [width * 0.22, width * 0.30, width * 0.48]
    if col_count == 2:
