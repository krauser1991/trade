...
MD_PATH = ROOT / "strategies/2026-07-06-本周题材聚焦与中报业绩评分.md"
PDF_PATH = ROOT / "output/pdf/2026-07-06-本周题材聚焦与中报业绩评分.pdf"
...
def build_styles():
    ...
    styles.add(ParagraphStyle(
        name="H2CN", fontName="CN", fontSize=14.1, leading=19.5,
        textColor=colors.HexColor("#0f3f6f"), spaceBefore=9, spaceAfter=6,
    ))
    styles.add(ParagraphStyle(
        name="H3CN", fontName="CN", fontSize=11.8, leading=16.2,
        textColor=colors.HexColor("#17324d"), spaceBefore=7, spaceAfter=4,
    ))
    styles.add(ParagraphStyle(
        name="BodyCN", fontName="CN", fontSize=8.9, leading=13.3,
        alignment=TA_LEFT, textColor=colors.HexColor("#222222"),
        spaceAfter=4.2,
    ))
    styles.add(ParagraphStyle(
        name="BulletCN", fontName="CN", fontSize=8.9, leading=13.3,
        leftIndent=12, firstLineIndent=-8, spaceAfter=3.6,
    ))
    styles.add(ParagraphStyle(
        name="CodeCN", fontName="CodeFont", fontSize=8.2, leading=11.5,
        leftIndent=8, rightIndent=8, textColor=colors.HexColor("#263238"),
        backColor=colors.HexColor("#f4f6f8"), borderPadding=6, spaceAfter=7,
    ))
    styles.add(ParagraphStyle(
        name="SmallCN", fontName="CN", fontSize=6.9, leading=9.3,
...
def table_widths(col_count: int, width: float):
    if col_count == 9:
        return [width * x for x in [0.07, 0.13, 0.08, 0.10, 0.09, 0.09, 0.09, 0.09, 0.26]]
    if col_count == 5:
        return [width * x for x in [0.17, 0.09, 0.19, 0.25, 0.30]]
    if col_count == 4:
        return [width * x for x in [0.16, 0.22, 0.26, 0.36]]
    if col_count == 3:
        return [width * 0.25, width * 0.33, width * 0.42]
    if col_count == 2:
