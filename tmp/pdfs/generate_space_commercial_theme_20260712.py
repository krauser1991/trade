...
MD_PATH = ROOT / "strategies/2026-07-12-商业航天题材持续性分析.md"
PDF_PATH = ROOT / "output/pdf/2026-07-12-商业航天题材持续性分析.pdf"
...
def build_styles():
    ...
    styles.add(ParagraphStyle(name="H2CN", fontName="CN", fontSize=13.4, leading=18.4, textColor=colors.HexColor("#0f3f6f"), spaceBefore=8, spaceAfter=5))
    styles.add(ParagraphStyle(name="H3CN", fontName="CN", fontSize=11.2, leading=15.6, textColor=colors.HexColor("#17324d"), spaceBefore=6, spaceAfter=4))
    styles.add(ParagraphStyle(name="BodyCN", fontName="CN", fontSize=8.6, leading=12.9, alignment=TA_LEFT, textColor=colors.HexColor("#222222"), spaceAfter=3.6))
    styles.add(ParagraphStyle(name="BulletCN", fontName="CN", fontSize=8.6, leading=12.9, leftIndent=12, firstLineIndent=-8, spaceAfter=3.1))
    ...
    styles.add(ParagraphStyle(name="SmallCN", fontName="CN", fontSize=6.45, leading=8.6, textColor=colors.HexColor("#333333")))
    return styles


def table_widths(col_count: int, width: float):
    if col_count == 5:
        return [width * x for x in [0.18, 0.16, 0.18, 0.18, 0.30]]
    if col_count == 4:
        return [width * x for x in [0.24, 0.20, 0.20, 0.36]]
    if col_count == 3:
        return [width * 0.28, width * 0.26, width * 0.46]
    if col_count == 2:
