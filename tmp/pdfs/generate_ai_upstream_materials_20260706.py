...
MD_PATH = ROOT / "strategies/2026-07-06-AI上游材料资金炒作顺序.md"
PDF_PATH = ROOT / "output/pdf/2026-07-06-AI上游材料资金炒作顺序.pdf"
...
def table_widths(col_count: int, width: float):
    if col_count == 6:
        return [width * x for x in [0.08, 0.25, 0.11, 0.15, 0.14, 0.27]]
    if col_count == 5:
        return [width * x for x in [0.16, 0.20, 0.22, 0.22, 0.20]]
    if col_count == 4:
        return [width * x for x in [0.18, 0.22, 0.24, 0.36]]
    if col_count == 3:
        return [width * 0.25, width * 0.34, width * 0.41]
    if col_count == 2:
