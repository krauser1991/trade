...
MD_PATH = ROOT / "2026-06-28-周末消息面与A股题材映射报告.md"
PDF_PATH = ROOT / "output/pdf/2026-06-28-周末消息面与A股题材映射报告.pdf"
...
def build_pdf():
    ...
    styles.add(ParagraphStyle(
        name="MetaCN", fontName="CN", fontSize=9, leading=14,
        alignment=TA_CENTER, textColor=colors.HexColor("#60646c"),
        spaceAfter=10,
    ))
    styles.add(ParagraphStyle(
    ...
    for line in lines:
        ...
        if stripped.startswith("# "):
            ...
            story.append(Paragraph("<b>" + clean_inline(stripped[5:]) + "</b>", styles["BodyCN"]))
        elif stripped.startswith("- "):
            ...
            story.append(Paragraph(clean_inline(stripped[2:]), styles["SmallCN"]))
        else:
